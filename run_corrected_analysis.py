...
# -*- coding: utf-8 -*-
"""
정확한 Case별 이벤트 기반 창고 분석 실행 스크립트
- 이중 카운트 방지
- 정확한 재고 산출
- 자동 엑셀 파일 생성
"""
...
def main():
    print("=== 정확한 Case별 이벤트 기반 창고 분석 시스템 ===")
    ...
    try:
        # 정확한 분석기 초기화
        print("\n🔍 정확한 분석기 초기화 중...")
        corrected_analyzer = CorrectedWarehouseAnalyzer(data_file, sheet_name='CASE LIST')
        
        # 정확한 종합 리포트 생성
        print("📊 정확한 종합 리포트 생성 중...")
        results = corrected_analyzer.generate_corrected_report(
        ...
        output_excel = os.path.join(output_dir, f'정확한_분석_{timestamp}_월별_창고_현장_입출고재고_집계.xlsx')
        ...
        print("\n🔧 개선된 알고리즘 특징:")
        print("- ✅ Case별 이벤트 타임라인 추적")
        print("- ✅ 창고 간 이동 시 이중 카운트 방지")
        print("- ✅ 정확한 월별 재고 산출")
        print("- ✅ 실제 이동 순서 반영")
